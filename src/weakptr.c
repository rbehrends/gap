...
#else
static inline Obj ElmWPObj(Obj list, UInt pos)
{
...
#ifndef USE_JULIA_GC
#define SET_ELM_WPOBJ(list,pos,val)             (ADDR_OBJ(list)[pos] = (val))
#else
static inline void SetElmWPObj(Obj list, UInt pos, Obj val)
{
...
#ifdef USE_JULIA_GC
static inline int IS_WEAK_DEAD_BAG(Bag bag)
{
    if (bag == NULL)
        return 1;
    if (!IS_BAG_REF(bag))
