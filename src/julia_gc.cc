...
treap_t * alloc_treap(void)
{
    ...
    if (treap_free_list) {
        ...
        result = (treap_t *) malloc(sizeof(treap_t));
    result->left = NULL;
    result->right = NULL;
    result->addr = NULL;
    result->size = 0;
...
static void TryMark(void * p)
{
    ...
    if (!p2) {
        ...
        // by a larger header.
        p2 = (jl_value_t *) treap_find(bigvals, (char *)p - 1);
        if (p2) {
    ...
    if (p2) {
        ...
        if (0) { // live.count((uintptr_t) p2) == 0) {
            ...
            if (t == datatype_mptr)
                printf("MPTR\n");
            else if (t == datatype_bag)
                printf("BAG\n");
            else if (t == datatype_largebag)
                printf("LBAG\n");
            abort();
        }
